...
def main():
    ...
    if not args.only_acktr and (args.lambda_ is None or args.margin is None or args.i_before is None):
        parser.error('--lambda, --margin and --alpha are required unless --only_acktr is present.')

    initial_max_load = 0.5
    initial_max_memory = 0.5

    if not args.only_acktr:
        modified_acktr_exps = [(x, y, z) for x in args.lambda_ for y in args.margin for z in args.i_before]
        num_exps_each_rep = len(modified_acktr_exps) if args.only_modified_acktr else len(modified_acktr_exps) + 1
    else:
        modified_acktr_exps = None
        num_exps_each_rep = 1
    ...
    parent_directory = os.path.join(
        os.getcwd(),
        '-'.join([args.env, 'acktr', datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')])
    )
    ...
    for i in range(args.num_repeat_times):
        need_acktr_exp = not args.only_modified_acktr
        need_modified_acktr_exp = not args.only_acktr

        num_committed_exps = 0
        if modified_acktr_exps is not None:
            modified_acktr_exps_to_do = modified_acktr_exps.copy()
        else:
            modified_acktr_exps_to_do = None
        while num_committed_exps < num_exps_each_rep:
            ...
            if need_acktr_exp:
                execute_training('acktr', new_available_gpu.pop(), parent_directory, i, config)
                need_acktr_exp = False
                num_committed_exps += 1

            if need_modified_acktr_exp and modified_acktr_exps_to_do is not None:
                while len(new_available_gpu) > 0 and len(modified_acktr_exps_to_do) > 0:
                    exp = modified_acktr_exps_to_do.pop()
                    execute_training(
                        'modified_acktr', new_available_gpu.pop(), parent_directory, i, config, exp[0], exp[1], exp[2]
                    )
                    num_committed_exps += 1
...
acktr_template = 'CUDA_VISIBLE_DEVICES={gpu_card} ' \
...
modified_acktr_template = 'CUDA_VISIBLE_DEVICES={gpu_card} ' \
                        'OPENAI_LOG_FORMAT=log,csv,tensorboard ' \
                        'python -m baselines.run ' \
                        '--alg={alg} ' \
                        '--env={env} ' \
...
gp.add_argument('--only_modified_acktr', action='store_true', help='Whether only to run modified acktr experiment')
gp.add_argument('--only_acktr', action='store_true', help='Whether only to run original acktr experiment')
...
def execute_training(alg, gpu_card, parent_directory, num_reps, config, lambda_=None, margin=None,
    ...
    if alg == 'acktr':
        config['log_path'] = os.path.join(parent_directory, '_'.join([config['env'], 'acktr', str(num_reps)]))
        config['alg'] = 'acktr'
        os.system(acktr_template.format(**config))
    elif alg == 'modified_acktr':
        ...
        os.system(modified_acktr_template.format(**config))
