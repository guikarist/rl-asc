...
def main():
    ...
    if not args.only_ppo and (args.lambda_ is None or args.margin is None or args.i_before is None):
        parser.error('--lambda, --margin and --i are required unless --only_ppo is present.')
    ...
    if not args.only_ppo:
        ...
        num_exps_each_epoch = len(modified_ppo_exps) if args.only_modified_ppo else len(modified_ppo_exps) + 1
    else:
